...
type thirdPartyServicesMock struct {
...
// get branch head
func (mock thirdPartyServicesMock) Head(repo *git.Repository) (*plumbing.Reference, error) {
	return mock.HeadHandler(repo)
}

// get branch worktree
func (mock thirdPartyServicesMock) Worktree(repo *git.Repository) (*git.Worktree, error) {
	return mock.WorktreeHandler(repo)
}

func (mock thirdPartyServicesMock) Fetch(repo *git.Repository) error {
	return mock.FetchHandler(repo)
}

func (mock thirdPartyServicesMock) Checkout(workingBranch *git.Worktree, branch string, ref *plumbing.Reference) (error, bool) {
	return mock.CheckoutHandler(workingBranch, branch, ref)
}

func (mock thirdPartyServicesMock) Add(workingBranch *git.Worktree, secretFile string) (plumbing.Hash, error) {
	return mock.AddHandler(workingBranch, secretFile)
}

func (mock thirdPartyServicesMock) Commit(workingBranch *git.Worktree, msg string, owner string) (plumbing.Hash, error) {
	return mock.CommitHandler(workingBranch, msg, owner)
}

func (mock thirdPartyServicesMock) CommitObject(repoGit *git.Repository, commit plumbing.Hash) (*object.Commit, error) {
	return mock.CommitObjectHandler(repoGit, commit)
}

func (mock thirdPartyServicesMock) Push(repoGit *git.Repository) error {
	return mock.PushHandler(repoGit)
}

func (mock thirdPartyServicesMock) PullRequest(ctx context.Context,
	...
	Context("CheckUserAccessRepo=>", func() {
		...
		Context("when user doesn't have rights to access  repo=>", func() {
			It("returns error and exits method", func() {
				...
				defer func() { GithubRepositories = GithubRepositoriesMock }()
				GithubRepositories = func(context.Context, string, string) (*github.Repository, *github.Response, error) {
					return new(github.Repository), new(github.Response), errors.New("Error in GithubRepositories method")
				}
				_, err := GitServiceObject.CheckUserAccessRepo("owner_test", "repo_test")
				Expect(err).To(Equal(errors.New("Error in GithubRepositories method")))
			})
		})
	})
	...
	Context("CreateBranchRepo=>", func() {
		Context("when providing repo details=>", func() {
			It("returns current branch and head branch", func() {
				gitServiceObj := thirdPartyServicesMock{}
		...
		Context("when folder path and changed contents are given=>", func() {
			It("save changes in the file", func() {
				...
				IoutilReadFile = func(string) ([]byte, error) {
					return []byte(currentSecretsFileContent), nil
				}
				...
				_ = json.Unmarshal([]byte(changesJSON), &secretsChangesMock)
			...
			Context("when file is  not found at the path=>", func() {
				It("returns error and exits the program", func() {
					...
					defer func() { IoutilReadFile = ioutilReadFileMock }()
					IoutilReadFile = func(string) ([]byte, error) {
						return []byte(currentSecretsFileContent), errors.New("Error in IoutilReadFile")
					}
					type updateParams struct {
						Repo    string                              `json:"repo" xml:"repo" form:"repo"`
						Owner   string                              `json:"owner" xml:"owner" form:"owner"`
			...
			Context("when indentation fails  while editing the file=>", func() {
				It("returns error and exits the program", func() {
					...
					JSONMarshalIndent = func(v interface{}, prefix, ident string) ([]byte, error) {
						return []byte(currentSecretsFileContent), errors.New("Error in JSONMarshalIndent")
					}
					type updateParams struct {
						Repo    string                              `json:"repo" xml:"repo" form:"repo"`
						Owner   string                              `json:"owner" xml:"owner" form:"owner"`
